# Tasks

~~1. Fix graceful shutting down of all crawlers as well as the UI and running script.~~

~~2. Make other crawlers faster, keep in mind rate limiting.~~

~~3. Try removing components like images from the pages to see the speed increase and raw data output.~~

~~4. Product collection is not showing in the UI during link collection phase.~~

5. For deployment how the scheduler going to run?

6. Apply scheduler code

~~7. Connect supabase as MCP, and check data insertion process.~~

8. Fix pause, refresh and resume logic.

9. Show database exit logs at the dashboard logs 