...
class AmazonDetailCrawler extends BaseCrawler {
  constructor(config = {}) {
    const memoryConfig = {
      ...
      memoryManagement: {
        ...
        pagePoolSize: 10, // Small pool for Amazon
    ...
    // Configuration
    this.category = config.category || 'tablet';
    this.categoryUrl = config.categoryUrl || 'https://www.amazon.in/s?i=computers&rh=n%3A1375458031&s=popularity-rank&page=1';
  ...
  /**
   * Restore URLs from checkpoint
   */
  restoreCheckpointUrls() {
    if (this.productLinks.length > 0) {
      this.productLinks.forEach(url => {
        this.seenUrls.add(url);
      });
