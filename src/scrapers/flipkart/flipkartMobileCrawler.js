...
class FlipkartCrawler extends BaseCrawler {
  constructor(config = {}) {
    const defaultConfig = {
      ...
      maxProducts: 5,
      // Disable request interception for related products to work
      disableRequestInterception: true,
    ...
    if (!this.checkpoint.productLinks) this.checkpoint.productLinks = [];
    if (!this.checkpoint.relatedLinks) this.checkpoint.relatedLinks = [];
    if (this.checkpoint.lastProcessedIndex === undefined) this.checkpoint.lastProcessedIndex = -1;
    if (this.checkpoint.lastRelatedIndex === undefined) this.checkpoint.lastRelatedIndex = -1;
    if (!this.checkpoint.failedProducts) this.checkpoint.failedProducts = [];
    if (!this.checkpoint.pagesScraped) this.checkpoint.pagesScraped = [];
    if (this.checkpoint.lastPageScraped === undefined) this.checkpoint.lastPageScraped = 0;

    // Related products are now added directly to productLinks array
    // No separate checkpoint structure needed
    ...
    // Debug logging
    this.logger.info(`Related products config: enabled=${this.relatedProductsConfig.enabled}, maxPerProduct=${this.relatedProductsConfig.maxPerProduct}`);

    // Set up seenUrls for deduplication
    this.seenUrls = new Set();

    // Load existing URLs from both arrays for global deduplication
    [...this.checkpoint.productLinks, ...this.checkpoint.relatedLinks].forEach(url => {
      const normalizedUrl = this.normalizeFlipkartUrl(url);
  ...
  async start() {
    try {
      ...
      // Related products are now processed automatically as part of the main productLinks array
  ...
  async processProductWithRetry(url, index, extractRelated) {
    let lastError;
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const productData = await this._scrapeProductDetail(url, extractRelated);
  ...
  async scrapeProductDetails() {
    ...
    for (let i = startIndex; i < endIndex; i += concurrent) {
      ...
      for (let j = i; j < batchEnd; j++) {
        ...
        // Extract related products only for main products (not for related products)
        const isRelatedProduct = this.relatedProductUrls.has(url);
        const shouldExtractRelated = !isRelatedProduct;
        batchPromises.push(this.processProductWithRetry(url, j, shouldExtractRelated));
      }
  ...
  async processProductArray(urlArray, indexKey, description, extractRelated) {
    ...
    for (let i = startIndex; i < endIndex; i += concurrent) {
      ...
      for (let j = i; j < batchEnd; j++) {
        ...
        batchPromises.push(this.processProductWithRetry(url, j, extractRelated));
      }
  ...
  async _scrapeProductDetail(url, extractRelated = false) {
    const page = await this.newPage();
    try {
      await page.setJavaScriptEnabled(true);
      ...
      if (extractRelated && this.relatedProductsConfig.enabled) {
        this.logger.info(`🔗 Extracting related products for: ${url}`);
        await this._extractRelatedProducts(page, url);
      }

      const result = { url, ...productData };
      if (!extractRelated) {
        result.isRelatedProduct = true;
  ...
  async _extractRelatedProducts(page, parentUrl) {
    try {
      ...
      $('.pq8uUv').each((index, element) => {
        ...
        if (jOp9dbDiv.length > 0) {
          ...
          this.logger.debug(`  Found section: "${text}"`);

          // Check if this section contains "Similar Products" or "Bought Together"
          if (text === 'Similar Products') {
            // Extract anchor tags from this specific section
      ...
      // Normalize and deduplicate URLs, then add to main productLinks array
      ...
      for (const url of allRelatedUrls) {
        ...
        if (normalizedUrl && !this.seenUrls.has(normalizedUrl)) {
          this.seenUrls.add(normalizedUrl);

          // Add to main productLinks array (not relatedLinks)
          this.checkpoint.productLinks.push(normalizedUrl);
          this.productLinks.push(normalizedUrl);
      ...
      this.logger.debug(`  Added ${addedCount} new related products to main queue`);
