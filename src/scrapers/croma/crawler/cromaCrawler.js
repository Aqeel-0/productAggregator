...
class ChromeCrawler extends BaseCrawler {
  ...
  async newPage() {
    ...
    await page.setUserAgent(this.config.userAgent || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36');
    await page.setViewport({ width: 1366, height: 768 });
    await page.setRequestInterception(true);
    page.on('request', (req) => {
      const rt = req.resourceType();
      if (['font', 'media', 'image'].includes(rt)) {
        req.abort();
      } else {
        req.continue();
      }
    });
    this.activePagesCount = (this.activePagesCount || 0) + 1;
    return page;
  }
  ...
  async start() {
    try {
      ...
      const totalProducts = this.maxProducts ? Math.min(this.maxProducts, collected): collected
