...
class AmazonValidator {
  constructor() {
    this.thresholds = {
      price_current: 0.15, // Max 15% missing
      brand: 0.20,
      model: 0.20,
      images: 0.10,
      ram: 0.25,
...
if (require.main === module) {
  const filePath = process.argv[2] || path.join(__dirname, '../../../parsed_data/amazon_normalized_data.json');
  const validator = new AmazonValidator();
  const result = validator.validate(filePath);
  console.log(JSON.stringify(result, null, 2));
  if (!result.success) process.exit(1);
}

module.exports = AmazonValidator;
