...
if (require.main === module) {
  const filePath = process.argv[2] || path.join(__dirname, '../../../parsed_data/croma_normalized_data.json');
  const validator = new CromaValidator();
  const result = validator.validate(filePath);
  console.log(JSON.stringify(result, null, 2));
  if (!result.success) process.exit(1);
}

module.exports = CromaValidator;
