...
if (require.main === module) {
  const filePath = process.argv[2] || path.join(__dirname, '../../../parsed_data/flipkart_normalized_data.json');
  const validator = new FlipkartValidator();
  const result = validator.validate(filePath);
  console.log(JSON.stringify(result, null, 2));
  if (!result.success) process.exit(1);
}

module.exports = FlipkartValidator;
